...
# Likelihood and MLE for ML classifications as independent variables (linear regression) -----------------
...
# Likelihood and MLE for ML classification as outcomes (logistic regression) -----------------
...
MLE_logit <- function(formula, data, fnr = 0, fpr = 0) {
  ...
  optimModel <- optim(vBetfnr, logLikelihoodLogit,
                     ...
                     method = 'L-BFGS-B',## does not work when FNR and FPR are both high
                     # method = 'BFGS', ## does not work when FPR is high and FPR is low; or vice versa
                     lower = rep(inff, ncol(mX)), upper = rep(uppf, ncol(mX)),
...
# Likelihood and MLE for ML classification as outcome (linear regression or linear prob model or LPM) -----------------
...
#' @return if return_boot_samples is false, return mean/standard error/quantile statistics from bootstrap sample only; otherwise return a list, the first element of which is mean/standard error/quantile statistics from bootstrap sample , and the second ele,ent of which is `nboot` samples from bootstrap draws
#' @export
MLE_logit_bootstrap <- function(formula, data, tl , pl, nboot = 10000, return_boot_samples = FALSE) {
  ...
  # d = data.frame("Estimate" = t(est),  "Std. Error" = t(sdd), "z value" = t(tv),  "Pr(>|z|)" = t(pv), check.names = FALSE)
  # d = data.frame("Estimate" = est,  "Std. Error" = sdd, "z value" = tv,  "Pr(>|z|)" = pv, check.names = FALSE)
  d = data.frame("Estimate" = est,  "Std. Error" = sdd, check.names = FALSE)
  d = rbind(d, data.frame("Estimate" = c(fnr, fpr),  "Std. Error" = c(fnr.sd, fpr.sd), check.names = FALSE))

  # return coefficient estimates as well as raw res
  if (return_boot_samples ){
    return (list(bayesian = d, raw_error = err))
    }else{
  return (d)
    }
}
...
#' @param working_dir directory to store JAGS temporary files
